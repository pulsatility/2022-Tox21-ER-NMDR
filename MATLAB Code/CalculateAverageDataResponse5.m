...
responseDataForReplicatedExperimentsWithPubIDRankOrder] = CalculateAverageDataResponse5(Data_cells)
...
for i = 1:1:length(responseDataWithRankAndPubchemSID_sorted(:,1))
    if (i > 1)
        if (responseDataWithRankAndPubchemSID_sorted(i,2) == responseDataWithRankAndPubchemSID_sorted(i-1,2))
            ...
            if (contains(sampleDataType(responseDataWithRankAndPubchemSID_sorted(i,1)), "set"))
