...
%% Input section
%Read the Tripod txt file as strings
...
%% Plot histograms and scatter for response and viablity data
...
%% Screen out the chemicals with low activity
...
%% Applying moving average to reduce the local effect
...
%% Recursive, correlation-based clustering
...
while (length(current_chemical_set) > 0)
    ...
    for i = 1:1:length(current_chemical_set_exclude_chosen_one)
        ...
        if (corr(current_dose_response_curve',...
                    compared_dose_response_curve') > 0.75)   
            Cluster_ID_Matrix(k,position_in_column) = current_chemical_set_exclude_chosen_one(i);
    ...
    % find the positions of clusterd chemicals in the current chemical set;
    % and then exclude them from the set
    for i = 1:1:length(Cluster_ID_Matrix_vector)
        Cluster_ID_Matrix_vector_Index(i) = find(current_chemical_set == Cluster_ID_Matrix_vector(i));
    end
...
%% Identify chemicals active in cytotoxicity, luciferase inhibition, autofluoresecence assays
...
%% Plot chemicals active in luciferase inhibition and autofluoresecence assays
...
%% Filter out U-shaped CRCs where the compounds are active in cytotoxicity, luciferase inhibition, autofluoresecence assays
...
%% Filter out Bell-shaped CRCs where the compounds are active in cytotoxicity, luciferase inhibition, autofluoresecence assays
