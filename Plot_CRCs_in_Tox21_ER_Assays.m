...
title(chemical_name);
    ...
    %legend(['ID=' num2str(SID)],'Location','southwest');
    %legend(['Compound name=' str2num(chemical_name)],'Location','northwest');

    sample_number = length(chemical_index_in_full_response_data);
    if(hasIntereferenceData == 1)
        legend ('Activity','Cytotoxicity','Luciferase inhibition')
    end
    if(hasAutofluoresenceData == 1)
        legend ('Activity','Cytotoxicity','Autofluoresecence')
    end

title(chemical_name,['Assay ' num2str(j),'; ','replication =  ' num2str(sample_number)]);
legend('boxoff')   
